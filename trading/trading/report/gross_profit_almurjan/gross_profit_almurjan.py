...
def execute(filters=None):
	...
	group_wise_columns = frappe._dict({
		"invoice": ["parent", "customer", "customer_group", "posting_date","item_code", "item_name","item_group", "brand", "description", \
			...
			"buying_amount","val_amount","vgp", "gross_profit", "gross_profit_percent", "project"],
		"item_code": ["item_code", "item_name", "brand", "description", "qty", "base_rate",
			"buying_rate", "base_amount", "buying_amount","val_amount","vgp", "gross_profit", "gross_profit_percent"],
		"warehouse": ["warehouse", "qty", "base_rate", "buying_rate", "base_amount", "buying_amount","vgp",
			"gross_profit", "gross_profit_percent"],
		"brand": ["brand", "qty", "base_rate", "buying_rate", "base_amount", "buying_amount","val_amount","vgp",
			"gross_profit", "gross_profit_percent"],
		"item_group": ["item_group", "qty", "base_rate", "buying_rate", "base_amount", "buying_amount","val_amount","vgp",
			"gross_profit", "gross_profit_percent"],
		"customer": ["customer", "customer_group", "qty", "base_rate", "buying_rate", "base_amount", "buying_amount",
			"val_amount","vgp","gross_profit", "gross_profit_percent"],
		...
		"sales_person": ["sales_person", "allocated_amount", "qty", "base_rate", "buying_rate", "base_amount", "buying_amount",
			"val_amount","vgp","gross_profit", "gross_profit_percent"],
		"project": ["project", "base_amount", "buying_amount","val_amount","vgp", "gross_profit", "gross_profit_percent"],
		"territory": ["territory", "base_amount", "buying_amount","val_amount","vgp", "gross_profit", "gross_profit_percent"]
	})

	columns = get_columns(group_wise_columns, filters)
	# print("gross_profit_data",gross_profit_data.grouped_data)
	for idx, src in enumerate(gross_profit_data.grouped_data):
		# print("idx.src",idx,src)
		row = []
...
def get_columns(group_wise_columns, filters):
	...
	column_map = frappe._dict({
		...
		"val_amount": _("Valuation Amount") + ":Currency/currency:100",
		"vgp": _("VGP") + ":Currency/currency:100",
...
class GrossProfitGenerator(object):
	...
	def process(self):
		...
		for row in self.si_list:
			...
			if row.qty:
				...
				row.val_amount = flt(row.buying_rate * row.qty,self.float_precision)
				row.vgp = flt(row.base_amount - row.val_amount,self.float_precision)
			else:
				row.buying_rate, row.base_rate, row.val_amount,row.vgp = 0.0, 0.0, 0.0, 0.0
	...
	def set_average_rate(self, new_row):
		...
		new_row.val_amount = flt(new_row.buying_rate * new_row.qty, self.float_precision) if new_row.buying_rate else 0
		new_row.vgp = flt(new_row.base_amount - new_row.val_amount, self.float_precision)
		return new_row
